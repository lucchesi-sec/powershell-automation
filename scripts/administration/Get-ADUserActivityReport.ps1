...
.PARAMETER Days
    The number of days to look back for activity data. For example, a value of 30 will include
    all activity within the last 30 days. The default is 30.
.PARAMETER OutputPath
    The directory where the generated reports will be saved. If not specified, it defaults to
    `$env:TEMP\ADReports`.
.PARAMETER Format
...
.PARAMETER SecurityThreshold
    The number of failed logon attempts to flag as suspicious. Any user with a `BadPwdCount`
    greater than or equal to this value will be included in the 'SuspiciousUsers' section of
    the SecurityFocus report. The default is 5.
.PARAMETER EmailReport
    If specified, the script will email the generated report to the recipients configured in
    `config/email.json`.
.EXAMPLE
